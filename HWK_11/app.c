...
unsigned char data[14]; //arrays for IMU data
short real_data[7];
unsigned char data_z[2];
short real_data_z;
char letter;
int arrNumbers[9] = {0};
float A = 0.4;
...
int newAvg = 0;
int sum = 0;
float a = 0.0338,b = 0.2401,c = 0.4521,d = 0.2401,e = 0.0338;
int len_array = sizeof(arrNumbers) / sizeof(int);
...
void APP_Tasks(void) {
    ...
    switch (appData.state) {
        ...
        case APP_STATE_SCHEDULE_WRITE:
            ...
            I2C_read_multiple(SLAVE_ADDR, OUTZ_L_XL, data_z , 2);
            //for(i=0;i<2;i++){ //Combine 14 8bit bytes into 7 16bits shorts
            real_data_z = (data_z[1] << 8) | data_z[0];
            //}
            
            //MAF Filter
            int z_MAF = movingAvg(arrNumbers, &sum, pos, len_array, (int) real_data_z);
            pos++;
            if (pos >= len_array){
              pos = 0;
            }
            ...
            //IIR Filter
            float z_IIR = old_z*A+real_data_z*B;
            old_z = real_data_z;
            
            //FIR Filter
            float z_FIR = a*arrNumbers[4]+b*arrNumbers[3]+c*arrNumbers[2]+d*arrNumbers[1]+e*arrNumbers[0];
                   
            
           
            len = sprintf(dataOut, "%d  %d  %d  %3.2f  %3.2f \r\n",j,real_data_z, z_MAF,z_IIR,z_FIR);
